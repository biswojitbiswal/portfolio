  ...
  return (
    <div className="relative mx-auto w-full max-w-[31rem]">
      <div className="group relative aspect-square w-full">
        ...
        <DotPattern className="top-[10%] left-[2%]" />
        <DotPattern className="bottom-[8%] left-[8%]" />
        <DotPattern className="right-0 bottom-[23%]" />
        ...
        {/* Circuit decoration */}
        <svg aria-hidden="true" viewBox="0 0 80 90" className="absolute top-[14%] right-[3%] h-20 w-16 text-technical/60">
          <path d="M14 4v34c0 7 5 12 12 12h30v25" fill="none" stroke="currentColor" strokeWidth="1" strokeDasharray="3 3" />

          <rect x="7" y="1" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.5" />

          <rect x="50" y="74" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.5" />
        ...
        {/* Small lower square */}
        <span aria-hidden="true" className="absolute bottom-[10%] left-[32%] size-4 border border-technical/60">
          <span className="absolute top-1/2 left-1/2 size-1 -translate-x-1/2 -translate-y-1/2 bg-technical" />
        </span>

        {/* Small right circle */}
        <span aria-hidden="true" className="absolute right-[1%] bottom-[31%] size-2.5 rounded-full border border-technical/70" />
      </div>
    </div>
  );
}
...
function DotPattern({ className = "" }: { className?: string }) {
  return (
    <div aria-hidden="true" className={`absolute z-0 grid grid-cols-6 gap-2 ${className} `}>
      {Array.from({ length: 30 }).map((_, index) => (
        <span key={index} className="size-1 rounded-full bg-grid-dot shadow-sm shadow-technical/10" />
      ))}
    </div>
  );
}
