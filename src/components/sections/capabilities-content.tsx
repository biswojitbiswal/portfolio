...
export type CapabilitiesData = {
  ...
  approachItems: { _key: string; title: string }[] | null;
};
...
export function CapabilitiesContent({ data }: { data: CapabilitiesData }) {
  const capabilities = useMemo<Capability[]>(() => (data.capabilityItems ?? []).map((item) => ({
    id: item._key,
    title: item.title,
    description: item.description,
    icon: capabilityIcons[item.icon] ?? Braces,
    examples: item.examples ?? [],
  })), [data.capabilityItems]);
  const engineeringApproach = data.approachItems ?? [];
  const [activeId, setActiveId] = useState<string | undefined>(capabilities[0]?.id);
  ...
  const activeCapability = useMemo(() => capabilities.find((capability) => capability.id === activeId) ?? capabilities[0], [activeId, capabilities]);

  const compactCapabilities = useMemo(() => capabilities.filter((capability) => capability.id !== activeCapability?.id), [activeCapability, capabilities]);
    ...
    <section id="capabilities" aria-labelledby="capabilities-heading" className="relative scroll-mt-24 overflow-hidden bg-background py-12 sm:py-12 lg:py-14">
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12 xl:px-16">
        <div className="hidden items-start gap-6 lg:grid lg:grid-cols-[minmax(300px,0.9fr)_minmax(0,1.8fr)] xl:grid-cols-[360px_minmax(0,1fr)] xl:gap-8">
          {/* Left introduction */}
          <div>
            <p className="font-mono text-xs font-semibold tracking-[0.12em] text-technical uppercase sm:text-sm">&lt; {data.sectionLabel} /&gt;</p>
            ...
            {engineeringApproach.length > 0 && <div className="mt-6">
              <p className="font-mono text-xs font-semibold tracking-[0.12em] text-technical uppercase">{data.approachLabel}</p>

              <ul className="mt-2 space-y-3">
                {engineeringApproach.map((item) => (
                  <li key={item._key} className="flex items-center gap-2 text-sm text-muted-foreground">
      ...
      <MobileCapabilities data={data} capabilities={capabilities} />
    </section>
  );
}

function MobileCapabilities({ data, capabilities }: { data: CapabilitiesData; capabilities: Capability[] }) {
  const [openId, setOpenId] = useState<string | null>(null);

  return (
    <div className="mx-auto w-full max-w-xl px-5 sm:px-8 lg:hidden">
      <header>
        <p className="font-mono text-xs font-semibold tracking-[0.12em] text-technical uppercase">&lt; {data.sectionLabel} /&gt;</p>

        <h2 className="mt-3 text-3xl leading-tight font-bold tracking-[-0.035em] text-foreground">{data.heading}</h2>

        <p className="mt-3 text-sm leading-6 text-muted-foreground">{data.shortDescription}</p>
      </header>
